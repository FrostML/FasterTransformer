...
public:
  TransformerSampling(const IAllocator &allocator, const int batch_size,
                   const int seq_len,
                   ...
                   const bool normalization_before = true) : allocator_(allocator)
  {
    ...
    decoder_ = new OpenTransformerDecoder<OpType_>(batch_size, memory_max_seq_len,
...

{
  ...
  DataType_* data = new DataType_[dims];
  cudaMemcpy(data, K_cache_[0], sizeof(DataType_) * dims, cudaMemcpyDeviceToHost);
  float sum = 0.0f;
    ...
    for (int step = 1; step <= args_.seq_len_; ++step) {
      ...
      for (int layer = 0; layer < args_.decoder_layers_; ++layer)
      {
        ...
        */
        ...
        check_cuda_error(cudaGetLastError());
#endif
        decoder_->forward(from_tensor_[from_id], nullptr,
